...
import "log"
import "base/p/flagfile"
import (
	"errors"
	"flag"
)

var common_flag string = ""
var flagfile_name string = "flagfile"

func init() {
	flag.StringVar(&flagfile_name, "flagfile", "", "flagfile, default ./flagfile -> conf/flagfile -> ../conf/flagfile")
	flag.StringVar(&common_flag, "common_flag", "", "common flagfile")
}
...
func InitAllModule() error {
	...
	flag.Parse()
	if common_flag != "" {
		flagfile.ParseFile(flag.CommandLine, common_flag)
	}
	if flagfile_name != "" {
		flagfile.ParseFile(flag.CommandLine, flagfile_name)
	} else {
		flagfile.Parse()
	}
	flag.Parse()
...
func (self *Module) SetInit(f callback) *Module {
	self.Initor = f
	return self
}
...
func Define(name string, initor func() error, fin func() error) *Module {
	mgr := GetModuleMgr()
	m := mgr.Registry(name)
	m.SetInit(initor)
	m.SetFin(fin)
